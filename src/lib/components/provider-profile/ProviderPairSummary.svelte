<script lang="ts">
	...
	const spread = $derived(currentRate?.spread ?? (buy > 0 && sell > 0 ? buy - sell : 0));
...
{#if mid > 0}
	<div class="font-normal space-y-2 text-[14px]" style="color: var(--text-secondary);">
		<p>
			...
			per {base}{#if rateMonthAgo > 0}, {moved}
				{Math.abs(changePct).toFixed(2)}% from {symbol}{fmt(rateMonthAgo)} a month ago{/if}.
			{#if !readMore}
				<button
		...
		{#if readMore}
			<div class="space-y-2" transition:slide={{ duration: 250 }}>
				{#if buy > 0 && sell > 0}
					<p>
						You pay <strong style="color: var(--text-primary);">{symbol}{fmt(buy)}</strong> to buy 1
						{base} here, and receive <strong style="color: var(--text-primary);">{symbol}{fmt(sell)}</strong>
						when you sell one — a spread of {symbol}{fmt(spread)}. That gap is what the round trip costs
						you, before any fees {providerName} charges separately.
					</p>
				{/if}
				<p>
					...
					for your {baseName}. At the current mid rate, {base}100 is worth roughly {symbol}{fmt(per100)}.
				</p>
				<p>
